...
def get_last_bingo_winner(data: List[str]) -> int:
    ...
    won_boards, last_board = dict(), None
    for draw in list(map(int, data[0].split(","))):
        ...
        for board_idx, board in enumerate(boards):
            ...
            if won:
                won_boards[board_idx] = score * draw
                last_board = board_idx
    # print(won_boards[last_board])
    return won_boards[last_board]
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        with open(input_file) as fin:
            print(get_last_bingo_winner(list(line.strip() for line in fin)))


assert get_last_bingo_winner([line for line in """1,2,3,6
...
22 11 13  6  5
 2  0 12  3  7
"""
assert get_last_bingo_winner([line for line in input.split("\n")]) == 1924
